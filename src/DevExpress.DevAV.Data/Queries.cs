...
namespace DevExpress.DevAV {
	...
	public static class QueriesHelper {
		static DateTime ActualDateTime { 
			get {
				if(Constants.TakeScreen)
					return Constants.Now.AddDays(720);
				return DateTime.Now.AddHours(24 - DateTime.Now.Hour);
			}
		}
		...
		public static decimal CustomSum<T>(this IEnumerable<T> query, Expression<Func<T, decimal>> selector) {
			var items = query.AsQueryable().Select(selector);
			return items.Any() ? items.AsEnumerable().Sum() : 0;
		}
		...
		public static IEnumerable<SaleAnalysisInfo> GetSaleAnalysis(IQueryable<OrderItem> orderItems) {
			return orderItems.Select(x => new SaleAnalysisInfo() {
				OrderDate = x.Order.OrderDate,
		...
		public static IEnumerable<string> GetStateNames(IQueryable<State> queryableStates, IEnumerable<StateEnum> states) {
			return queryableStates
#if NET
				.ToList()
#endif
				.Join(states, ss => ss.ShortName, s => s, (ss, s) => ss.LongName);
		}
		...
		public static IQueryable<ProductInfoWithSales> GetProductInfoWithSales(IQueryable<Product> products) {
			return products
				.Select(x => new ProductInfoWithSales {
					Id = x.Id,
					Name = x.Name,
					Cost = x.Cost,
		...
		public static IQueryable<CustomerInfoWithSales> GetCustomerInfoWithSales(IQueryable<Customer> customers) {
			return customers  
				.Select(x => new CustomerInfoWithSales {
				Id = x.Id,
				Name = x.Name,
		...
		public static void UpdateCustomerInfoWithSales(IEnumerable<CustomerInfoWithSales> entities, IQueryable<CustomerStore> stores, IQueryable<CustomerEmployee> employees, IQueryable<Order> orders) {
			foreach(var item in entities) {
				item.Init(
					...
					orders.Where(x => x.CustomerId == item.Id)
#if NET
					.AsEnumerable()
#endif
					.GroupBy(o => o.OrderDate.Month).Select(g => g.Sum(i => i.TotalAmount)).ToArray()
				);
			}
		}
		public static IQueryable<Order> GetOrdersForPeriod(IQueryable<Order> orders, Period period, DateTime dateTime = new DateTime()) {
			switch(period) {
				case Period.ThisYear:
					return orders.Where(o => o.OrderDate.Year == Constants.Now.Year);
				case Period.ThisMonth:
					return orders.Where(o => o.OrderDate.Month == Constants.Now.Month && o.OrderDate.Year == Constants.Now.Year);
		...
		public static Expression<Func<OrderItem, bool>> GetOrderItemsForPeriodFilter(Period period, DateTime dateTime = new DateTime()) {
			switch(period) {
				case Period.ThisYear:
					return x => x.Order.OrderDate.Year == Constants.Now.Year;
				case Period.ThisMonth:
					return x => x.Order.OrderDate.Month == Constants.Now.Month && x.Order.OrderDate.Year == Constants.Now.Year;
				case Period.FixedDate:
		...
		public static IEnumerable<CustomerStore> GetSalesStoresForPeriod(IQueryable<Order> orders, Period period = Period.Lifetime) {
			return QueriesHelper.GetOrdersForPeriod(orders, period)
#if NET
				.ToList()
#endif
				.GroupBy(o => o.Store).Select(g => g.Key).Distinct();
		}
...
#if NET
			return GetSaleMapItems(orderItems.Where(x => string.Equals(x.Order.Store.Address_City, city)), productId, period);
#else
		return GetSaleMapItems(orderItems.Where(x => string.Equals(x.Order.Store.Address.City, city)), productId, period);
#endif
		}
...
#if NET
			return GetSaleMapItemsByCustomer(orderItems.Where(x => string.Equals(x.Order.Store.Address_City, city)), customerId, period);
#else
		return GetSaleMapItemsByCustomer(orderItems.Where(x => string.Equals(x.Order.Store.Address.City, city)), customerId, period);
#endif
		}
		...
		public static IEnumerable<CustomerStore> GetDistinctStoresForPeriod(IQueryable<Order> orders, long customerId, Period period = Period.Lifetime) {
			return QueriesHelper.GetCustomerOrdersForPeriod(orders, period, customerId)
#if NET
				.ToList()
#endif
				.GroupBy(o => o.Store).Select(g => g.Key).Distinct();
		...
		public static IEnumerable<OrderItem> GetRevenueReportItems(IQueryable<OrderItem> orderItems) {
			bool hasItemsInCurrentMonth = orderItems.Where(x => x.Order.OrderDate.Month == Constants.Now.Month && x.Order.OrderDate.Year == Constants.Now.Year).Any();
			var dateOfLastOrder = orderItems.Max(x => x.Order.OrderDate);
			var revenueMonth = hasItemsInCurrentMonth ? Constants.Now.Month : dateOfLastOrder.Month;
			var revenueYear = hasItemsInCurrentMonth ? Constants.Now.Year : dateOfLastOrder.Year;
