    ...
    public partial class App : Application {
        ...
        static Assembly OnCurrentDomainAssemblyResolve(object sender, ResolveEventArgs args) {
            ...
            if(partialName == "entityframework" || partialName == "system.data.sqlite" || partialName == "system.data.sqlite.ef6") {
                ...
                return SafeTypeResolver.GetOrLoadAssemblyFrom(path);
            }
            return null;
        }
        #region DemoTester
        ...
        public static void Test(ThemedWindow window) {
            string testerPath = null;
            bool allowDemoTest = false;
            if (DevAVDataDirectoryHelper.IsClickOnce) {
                if(testTimer == null) {
                ...
                testerPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), string.Format("DevExpress.Reallife.Tester.v{0}.dll", AssemblyInfo.VersionShort));
                allowDemoTest = true;
            } else {
                var frameworkPrefix = Xpf.Core.Utils.NetVersionDetector.IsNetCore3() ? "NETCoreDesktop" : "Framework4";
                testerPath = string.Format(@"c:\DevExpress.Tester\{0}\DevExpress.Reallife.Tester.v{1}.dll", frameworkPrefix, AssemblyInfo.VersionShort);
                var args = Environment.GetCommandLineArgs();
                allowDemoTest = args.Any() && args.Contains(@"/testallexcept:");
            }
            if(File.Exists(testerPath) && allowDemoTest) {
                ExceptionHelper.IsEnabled = false;
                var assembly = SafeTypeResolver.GetOrLoadAssemblyFrom(testerPath);
                var type = SafeTypeResolver.GetKnownType(assembly, "DevExpress.Reallife.Tester.OutlookInspiredAppDemoTester");
                type.GetMethod("Create")?.Invoke(null, new object[] { window, true, DevAVDataDirectoryHelper.IsClickOnce });
            }
        }
#endregion
